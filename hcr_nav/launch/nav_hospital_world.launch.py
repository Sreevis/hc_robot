...
def generate_launch_description():
    ...
    package_dir=get_package_share_directory('hcr_gazebo')
    pkg_gazebo_ros = get_package_share_directory('gazebo_ros')
    world_file = os.path.join(package_dir,'worlds','hospital.world')

    hcr_nav_path = os.path.join(
    get_package_share_directory('hcr_nav'))
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        name='urdf_spawner',
        output='screen',
        arguments=["-topic", "/robot_description", "-entity", "hcr", "-x", "0.0", "-y", "10.0",  "-z", "0.0"]
    )
    ...
    robot_localization_node = launch_ros.actions.Node(
        ...
        output='screen',
        parameters=[os.path.join(hcr_nav_path, 'config/ekf.yaml'), {'use_sim_time': LaunchConfiguration('use_sim_time')}]
    )
